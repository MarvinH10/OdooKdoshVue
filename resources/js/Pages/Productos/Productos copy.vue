...
const loadCategories = async () => {
    try {
        const categoriesResponse = await axios.get("/categorias/traer");
...
const loadAttributes = async () => {
    try {
        ...
        if (storedAttributes) {
            allAttributes.value = JSON.parse(storedAttributes);
            console.log("Atributos cargados desde el localStorage.");
        } else {
            const attributesResponse = await axios.get("/atributos/traer");
            localStorage.setItem('attributes', JSON.stringify(attributesResponse.data));
            allAttributes.value = attributesResponse.data;
            console.log("Atributos cargados desde la API y almacenados.");
        }
    } catch (error) {
...
const fetchAttributeValues = async (id, index) => {
    try {
        const storedValues = localStorage.getItem(`attribute_${id}_values`);
        ...
        if (storedValues) {
            ...
            localStorage.setItem(`attribute_${id}_values`, JSON.stringify(response.data));
            // console.log(`Valores de atributo ${id} cargados desde la API y almacenados.`);
        }
