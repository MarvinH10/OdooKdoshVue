...
class ProductosController extends Controller
{
    ...
    public function traerValoresAtributos($atributoId)
    {

        $valores_atributos = ValorAtributo::where('atr_id', $atributoId)->get();

        if ($valores_atributos->isEmpty()) {
            return response()->json(['error' => 'No se encontraron valores para este atributo'], 404);
        }

        return response()->json($valores_atributos);
    }
